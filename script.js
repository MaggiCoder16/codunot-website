...
function setCookie(name, value, maxAgeDays) {
  ...
  const maxAge = typeof maxAgeDays === 'number' ? `; Max-Age=${maxAgeDays * 24 * 60 * 60}` : '';
  document.cookie = `${name}=${safeValue}${maxAge}; Path=/; SameSite=Lax`;
}
...
function loadPrefs() {
  const raw = window.localStorage.getItem(PREFS_KEY) || decodeURIComponent(getCookie(PREFS_KEY) || '');
...
function applyUiPrefs(prefs) {
  applyThemeMode(prefs.theme || 'system');
  document.documentElement.dataset.density = prefs.density || 'cozy';
...
function initThemeControl() {
  ...
  const initial = prefs.theme || 'system';

  const wrapper = document.createElement('label');
  wrapper.className = 'theme-control';
  wrapper.innerHTML = '<span>Theme</span>';
  ...
  applyUiPrefs(prefs);
  syncThemeSelects(initial);
}

function initPreferencesCenter() {
  const prefs = loadPrefs();
  ...
  function applyAndSave(next) {
    ...
    savePrefs(prefs);
    applyUiPrefs(prefs);
    syncThemeSelects(prefs.theme || 'system');
  }
  ...
  banner.innerHTML = [
    ...
    '<p>Choose how Codunot uses cookies for settings, analytics, and marketing.</p>',
    '</div>',
    '<div class="cookie-actions">',
    '<button class="btn ghost" type="button" data-action="reject">Reject</button>',
    '<button class="btn secondary" type="button" data-action="manage">Manage</button>',
    '<button class="btn primary" type="button" data-action="accept">Accept all</button>',
    '</div>'
  ].join('');
  ...
  modal.innerHTML = [
    ...
    '<p class="muted">Customize how Codunot remembers your settings.</p>',
    '</header>',
    '<div class="pref-grid">',
    '<label class="pref-toggle">',
    '<input type="checkbox" data-pref="preferences" />',
    '<span>Preferences cookies</span>',
    '<em>Theme, layout, and saved settings.</em>',
    '</label>',
    '<label class="pref-toggle">',
    '<input type="checkbox" data-pref="analytics" />',
    '<span>Analytics cookies</span>',
    '<em>Help us understand usage patterns.</em>',
    '</label>',
    '<label class="pref-toggle">',
    '<input type="checkbox" data-pref="marketing" />',
    '<span>Marketing cookies</span>',
    '<em>Personalize announcements and offers.</em>',
    '</label>',
    '</div>',
    '<div class="pref-row">',
    '<label>',
    '<span>Theme</span>',
    '<select class="theme-select" data-pref="theme">',
    ...
    '<button class="btn primary" type="button" data-action="save">Save preferences</button>',
    '</div>',
    '</div>'
  ].join('');
  ...
  function hydrateModal() {
    modal.querySelectorAll('input[data-pref]').forEach((input) => {
      const key = input.getAttribute('data-pref');
      input.checked = Boolean(prefs[key]);
    });

    modal.querySelectorAll('select[data-pref]').forEach((select) => {
      const key = select.getAttribute('data-pref');
      select.value = prefs[key] || DEFAULT_PREFS[key];
    });
  }
  ...
  function handleBannerClick(action) {
    if (action === 'accept') {
      ...
      setCookie('codunot_cookie_ack', '1', 365);
      banner.remove();
      return;
    }
  ...
  modal.querySelectorAll('button[data-action]').forEach((button) => {
    button.addEventListener('click', () => {
      ...
      if (action === 'save') {
        const next = {};
        modal.querySelectorAll('[data-pref]').forEach((field) => {
          const key = field.getAttribute('data-pref');
  ...
  const triggers = document.querySelectorAll('.js-open-preferences');
  ...
  if (footer && !footer.querySelector('.js-open-preferences')) {
    const button = document.createElement('button');
    button.type = 'button';
    button.className = 'footer-pref js-open-preferences';
    ...
    const targetColumn = footer.querySelector('.footer-col:last-child') || footer.querySelector('.footer-col');
    if (targetColumn) targetColumn.appendChild(button);
  }

  if (!getCookie('codunot_cookie_ack')) {
...
function initGateSessionCleanup() {
  ...
  document.addEventListener('click', (event) => {
    ...
    if (nextUrl.origin === window.location.origin) internalNav = true;
  }, { capture: true });
  ...
  window.addEventListener('pagehide', () => {
    ...
    if (navigator.sendBeacon) {
      navigator.sendBeacon('/__turnstile/logout');
    } else {
      fetch('/__turnstile/logout', { method: 'POST', keepalive: true });
    }
  });
}
...
function initAutoHideHeader() {
  ...
  const mediaQuery = window.matchMedia('(pointer: coarse) and (max-width: 1180px)');
  let lastScrollY = window.scrollY;
  let ticking = false;

  function showHeader() {
    navWrap.classList.remove('is-hidden');
    navWrap.classList.add('is-peeking');
  }

  function hideHeader() {
    navWrap.classList.remove('is-peeking');
    navWrap.classList.add('is-hidden');
  }

  function resetHeader() {
    navWrap.classList.remove('is-hidden', 'is-peeking');
  }

  function shouldKeepVisible() {
    return links.classList.contains('open') || Boolean(document.querySelector('.nav-dropdown.open'));
  }
  ...
  function updateHeader() {
    ...
    if (currentScrollY <= 20 || shouldKeepVisible()) {
      showHeader();
      lastScrollY = currentScrollY;
      return;
    }

    if (delta > 8 && currentScrollY > 120) hideHeader();
    else if (delta < -8) showHeader();
...
function initSupportForm() {
  ...
  // Prefer explicit endpoint for cross-origin Worker deployments.
  const endpoint = form.dataset.endpoint || form.getAttribute('action') || '/api/support';
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      if (!response.ok || payload.ok !== true) {
        const message = payload.error || 'Submission failed. Please try again.';
        throw new Error(message);
      }

      setStatus('Request sent! We will follow up soon.', 'success');
      form.reset();
